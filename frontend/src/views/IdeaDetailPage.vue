    ...
    <div v-if="item" class="bg-white rounded-2xl shadow-sm overflow-hidden border border-gray-100">
      ...
      <div class="px-5 sm:px-8 pt-4 pb-3 flex items-center justify-between flex-wrap gap-2 border-b border-gray-100 bg-gray-50/50">
        ...
        <div class="flex items-center gap-2">
          <PublishButton source-type="idea" :source-id="route.params.id" />
      ...
      <article class="px-5 sm:px-10 py-7">
        ...
        <div v-else-if="!item.title" class="py-10 text-center text-gray-300">（这条灵感暂无内容）</div>
...
async function load() {
  try {
    item.value = await ideaApi.get(route.params.id)
  } catch {
...
function goBack() {
  router.push({ path: '/ideas', query: { ...route.query } })
}

function edit() {
  router.push({ path: '/ideas', query: { edit: route.params.id } })
}
...
async function remove() {
  ...
  try {
    await ideaApi.remove(route.params.id)
